# Cleanup Script for Church Business Application
# Removes containers and images

Write-Host "🧹 Church Business Application - Cleanup" -ForegroundColor Yellow
Write-Host "=========================================" -ForegroundColor Yellow

# Stop and remove container
Write-Host "🔄 Stopping and removing container..." -ForegroundColor Cyan
$container = docker ps -a --filter "name=church-business-container" --format "{{.ID}}"
if ($container) {
...
$image = docker images --filter "reference=church-business-app" --format "{{.ID}}"
if ($image) {
