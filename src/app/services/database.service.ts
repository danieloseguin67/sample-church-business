...
export class DatabaseService {
   ...
   */
  getData<T>(endpoint: string, params?: Record<string, string | number | boolean>): Observable<ApiResponse<T>> {
   ...
   */
  postData<T>(endpoint: string, data: unknown): Observable<ApiResponse<T>> {
    const url = `${this.apiUrl}/${endpoint}`;
   ...
   * @param data - Request payload
   */
  updateData<T>(endpoint: string, data: unknown): Observable<ApiResponse<T>> {
    const url = `${this.apiUrl}/${endpoint}`;
    const options = { headers: this.getHttpHeaders() };
   ...
   */
  healthCheck(): Observable<ApiResponse<Record<string, unknown>>> {
